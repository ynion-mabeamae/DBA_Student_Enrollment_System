...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['delete_program'])) {
        ...
        $sql = "UPDATE tblprogram SET is_active = FALSE WHERE program_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $program_id);
        
        if ($stmt->execute()) {
            $_SESSION['message'] = "success::Program archived successfully!";
        } else {
            $_SESSION['message'] = "error::Error archiving program: " . $conn->error;
        }
    ...
    // RESTORE PROGRAM functionality
    if (isset($_POST['restore_program'])) {
        $program_id = $_POST['program_id'];
        
        $sql = "UPDATE tblprogram SET is_active = TRUE WHERE program_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $program_id);
        
        if ($stmt->execute()) {
            $_SESSION['message'] = "success::Program restored successfully!";
        } else {
            $_SESSION['message'] = "error::Error restoring program: " . $conn->error;
        }
...
// Count programs
$active_programs_count = $conn->query("SELECT COUNT(*) FROM tblprogram WHERE is_active = TRUE")->fetch_row()[0];
$archived_programs_count = $conn->query("SELECT COUNT(*) FROM tblprogram WHERE is_active = FALSE")->fetch_row()[0];
$total_programs = $show_archived ? $archived_programs_count : $active_programs_count;
?>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="program-status-toggle no-print">
            <a href="?page=programs" class="status-btn <?php echo !$show_archived ? 'active' : ''; ?>">
                <i class="fas fa-user-check"></i>
                Active Programs (<?php echo $active_programs_count; ?>)
            </a>
            <a href="?page=programs&show_archived=true" class="status-btn <?php echo $show_archived ? 'active' : ''; ?>">
                <i class="fas fa-archive"></i>
                Archived Programs (<?php echo $archived_programs_count; ?>)
            </a>
        </div>
        ...
        <div class="delete-confirmation" id="deleteConfirmation">
            <div class="confirmation-dialog">
                ...
                <p id="deleteMessage">Are you sure you want to delete this program? This action will move the program to archived records.</p>
                <div class="confirmation-actions">
        ...
        <div class="table-container">
            ...
            <div class="search-container">
                ...
                <div class="search-stats" id="searchStats">
                    Showing <?php echo $total_programs; ?> of <?php echo $total_programs; ?> 
                    <?php echo $show_archived ? 'archived' : 'active'; ?> programs
                </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    ?>
                    <tr class="<?php echo $show_archived ? 'archived-program' : ''; ?>">
                        <td>
                        ...
                        <td class="actions">
                            <?php if ($show_archived): ?>
                                <!-- Only show Restore button for archived programs -->
                                <form method="POST" style="display: inline;">
                                    <input type="hidden" name="program_id" value="<?php echo $program['program_id']; ?>">
                                    <button type="submit" name="restore_program" class="btn btn-success">
                                        <i class="fas fa-trash-restore"></i>
                                        Restore
                                    </button>
                                </form>
                            <?php else: ?>
                                <!-- Show Edit and Delete buttons for active programs -->
