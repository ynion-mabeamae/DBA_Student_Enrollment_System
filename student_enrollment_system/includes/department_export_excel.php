...
$total_pages = ceil($total_departments / $rows_per_page);

// Set headers for Excel download
header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment; filename="departments_report_' . date('Y-m-d') . '.xls"');
...
echo '<div class="report-title">DEPARTMENT MASTER LIST</div>';
echo '<div>Generated on: ' . date('F j, Y g:i A') . '</div>';
echo '<div>Total Pages: ' . $total_pages . ' | Total Departments: ' . $total_departments . '</div>';
echo '</div>';

// Summary section
echo '<div class="summary">';
echo '<strong>REPORT SUMMARY:</strong><br>';
echo 'Total Departments: ' . $total_departments . '<br>';
...
while($department = $departments->fetch_assoc()): 
    ...
    if ($row_count % $rows_per_page == 0 && $row_count > 0) {
        ...
        echo 'Department Management System | ' . date('F j, Y');
        echo '</div>';
    ...
    echo '<td>' . htmlspecialchars($department['dept_code']) . '</td>';
    echo '<td>' . htmlspecialchars($department['dept_name']) . '</td>';
    echo '</tr>';
    
    $row_count++;
    
    // Close table if it's the last row
    if ($row_count == $total_departments) {
        echo '</table>';
    }
endwhile;
...
// Final footer
echo '<div class="footer">';
echo '<strong>Page ' . $current_page . ' of ' . $total_pages . ' - Total Departments: ' . $total_departments . '</strong><br>';
